    ...
    try {
        ...
        async function antiSpam(ctx, delay = 10) {
            ...
            if (ctx.session.userTime.m === getTime(ctx.message.date).m) {
                ...
                let res =
                    60 - (ctx.session.userTime.s - getTime(ctx.message.date).s);
            ...
            if (dropUserID !== undefined && dropUser === null) {
                if (dropUserID) {
                    if (dropUserID.from_id < 0) {
                        if (state === 'rep')
                            return ctx.reply(`Cебе кинь &#128545;`);
                        if (state === 'res')
                            return ctx.reply(
                                `Пасибо, за это можешь себя похвалить ☺`
                            );
                    }
        ...
        async function giveRandomPost(ctx, groups, type) {
            ...
            try {
                ...
                if (err.response.error_code === 29) {
                    ...
                    ctx.reply(
                        '☢ Блин блинский, не могу выдать [giveRandomPost]'
                    );
                }
                console.error(err);
            }
        ...
        async function getPictureFromAlbum(ctx, text, albumID = 275086127) {
            try {
                ...
                console.error(err);
                ctx.reply(
                    '☢ Блин блинский, не могу выдать [getPictureFromAlbum]'
                );
            }
        }
        ...
        async function sendUserWhoHe(ctx, arPerson, albumID = 275086127) {
            try {
                ...
                const picture = await getPictureFromAlbum(
                    ctx,
                    randomItem,
                    albumID
                );
        ...
        bot.command(/^!(монетка|м)$/i, async (ctx) => {
            ...
            ctx.reply(
                `у ${user.first_name} ${
                    side === 0 ? 'выпала Решка' : 'выпал Орёл'
                }`
            );
        });
        // Игра "ROLL"
        ...
        bot.command(/^!8\s[a-zа-я0-9\W]+$/, (ctx) => {
            ...
            return ctx.reply(
                '🎱 ' + arAnswers[getRandomInt(0, arAnswers.length)]
            );
        });
        //==========================================================================================
        // CHALLENGES
        ...
        bot.command(
            ...
            async (ctx) => {
                ...
                const dropUser = ctx.message.text
                    .match(/@[\w-]+/gi)[0]
                    .slice(1);
        ...
        bot.command(
            /!(report|respect|res|rep)\s[a-zа-я0-9\W]+/i,
            async (ctx) => {
        ...
        bot.command(/^!(anec old|анек олд|анекдот олд)$/i, async (ctx) => {
            ...
            async function getAnecdote() {
                try {
                    ...
                    console.error(err);
                    ctx.reply(
                        '&#9762; Блин блинский, не могу выдать [anec_old]'
                    );
                }
            }
        ...
        bot.command(
            /(мужика в пиве|мужик в пиве|пиво в мужике)/i,
            async (ctx) => {
                const spam = await antiSpam(ctx, 5);
                if (spam) return;
        ...
        bot.command(
            /(п(у|а)д(ж|жик)|(п|р)(у|а)дге|pudge|быдло)/i,
            async (ctx) => {
                const spam = await antiSpam(ctx, 5);
                if (spam) return;
        ...
        bot.command(/^!вовёр$/i, async (ctx) => {
            ...
            ctx.reply('Вовёр 🍍', picture);
        });
        bot.command(/^!дэнч$/i, async (ctx) => {
            const spam = await antiSpam(ctx, 5);
            if (spam) return;
            const picture = await getPictureFromAlbum(ctx, 'дэнч');
            ctx.reply('Даня 🍺', picture);
        });
        bot.command(/^!vlados$/i, async (ctx) => {
            const spam = await antiSpam(ctx, 5);
            if (spam) return;
            const picture = await getPictureFromAlbum(ctx, 'vlados');
            ctx.reply('Владос, С Днём Рождения! 🤙🏻', picture);
        });
            ...
            try {
                ...
                const randomPerson =
                    conversation.profiles[
                        getRandomInt(0, conversation.profiles.length)
                    ];
                const randomGachi =
                    arGachi[getRandomInt(0, arGachi.length - 1)];
                ctx.reply(
        ...
        async function showStatus(ctx, user) {
            const neededUser = await getNeededUser(
                ctx,
                user,
                ctx.message.peer_id
            );
        ...
        bot.command(/^!21\supdate\sgame$/, (ctx) => {
            async function updateGame21(ctx) {
                try {
                    ...
                    console.error(err);
                    ctx.reply(
                        '&#9762; Блин блинский, сбой какой-то [update_21]'
                    );
                }
            }
        ...
        bot.command(/^!inv$/, async (ctx) => {
            ...
            try {
                ...
                if (conversation.profiles.length === 1) {
                    return ctx.reply(
                        '☢ Данная команда работает только в беседах!'
                    );
                }
                const user = await getUser(userID);
                ...
                inventoryItems = items
                    .map((el) => {
                        const symbol = arItems.filter((i) => i.name === el)[0]
                            .symbol;
        ...
        // -=-=-=-ZXC-=-=-=-=-
        async function zxcSender(ctx, id = false) {
            try {
                const conversation = await bot.execute(
                    'messages.getConversationMembers',
                    {
                ...
                if (conversation.profiles.length < 2) {
                    throw '☢ Данная команда работает только в беседах!';
                }

                let dropUser = null;
                const sender = await getUser(ctx.message.from_id);
                const conversationID = ctx.message.peer_id;

                if (id) {
                    dropUser = ctx.message.text.match(/@[\w-]+/gi)[0].slice(1);
                    dropUser = await getUser(dropUser);
                } else {
                ...
                if (sender.screen_name === dropUser.screen_name)
                    throw 'Ты шо zxc-🤡?';
                //---------------------------------------------------
                ...
                let existSender = existRoom.list.filter(
                    (el) => el.user === sender.screen_name
                )[0];

                let existUser = existRoom.list.filter(
                    (el) => el.user === dropUser.screen_name
                )[0];

                function ObjUserInDB(user) {
                    return {
                ...
                if (!existSender) {
                    ...
                    existSender = existRoom.list.filter(
                        (el) => el.user === sender.screen_name
                    )[0];
                }
                ...
                const success = getRandomInt(0, 2) === 1 ? true : false;
                const imgPause = await getPictureFromAlbum(ctx, 'pause');
                await bot.sendMessage(conversationID, '', imgPause);
                ctx.reply(`${existSender.firstName} is pausing in 3`).then(
                    () => {
                        setTimeout(() => {
                            ctx.reply(
                                `${existSender.firstName} is pausing in 2`
                            ).then(() => {
                                setTimeout(() => {
                                    ctx.reply(
                                        `${existSender.firstName} is pausing in 1`
                                    ).then(() => {
                                        ctx.reply(
                                            `${existSender.firstName}-гуль: ?`
                                        ).then(() => {
                                            setTimeout(() => {
                                                ctx.reply(
                                                    `😡 Недовольный ${existUser.firstName} resumed the game.`
                                                ).then(() => {
                                                    setTimeout(async () => {
                                                        if (success) {
                                                            await room.updateOne(
        ...
        bot.command(/^!zxc\s\[[\w]+\W@[\w-]+\]$/, async (ctx) => {
            await zxcSender(ctx, true);
        });
        bot.command(/^!zxc$/, async (ctx) => {
            await zxcSender(ctx);
        });
        //-=-=-=-=-=-=-=-=-=-=-
        // LOOT USE
            ...
            if (ctx.message.attachments.length) {
                ...
                if (attachemnt.type === 'photo') {
                    ...
                    if (conversation.profiles.length > 1) {
                        ...
                        if (randomItem !== null) {
                            randomItem =
                                randomItem[getRandomInt(0, randomItem.length)];
                            existRoom = await room.findOne({
                                room: roomID,
                                'list.user': user.screen_name,
                            });
            ...
            if (ctx.message.payload) {
                ...
                async function endGame21(room, arDelRoom) {
                    ...
                    if (arTopPlayers[0].score === arTopPlayers[1].score) {
                        ...
                        winner = arPlayersEqual.reduce(
                            (acc, current) => {
                                if (
                                    new Date(acc.date) < new Date(current.date)
                                ) {
                                    return acc;
                                } else {
                                    return current;
                ...
                async function startRouletteGame(roll, callback = null) {
                    try {
                        ...
                        if (!existRoom)
                            return ctx.reply(
                                `🔫 ${user.first_name}, ты не взял револьвер!`
                            );
                        ...
                        if (!existPlayer.bullet > 0 || roll) {
                            await room.updateOne(
                                {
                                    room: conversationID,
                                    'roulette.players.user': userID,
                                },
                                {
                                    $set: {
                                        'roulette.players.$.bullet': getBullet(
                                            players.length
                                        ),
                                    },
                                }
                        ...
                        console.error(err);
                        ctx.reply(
                            '☢ Блин блинский, сбой какой-то [startRouletteGame]'
                        );
                    }
                }
                ...
                if (payload.action === 'throwGlove') {
                    ...
                    await useLoot(
                        ...
                        async (owner, user, existUser, sender, senderGen) => {
                                ...
                                if (user.inventory.glove) {
                                    await room
                                        .updateOne(
                                            ...
                                            {
                                                $set: {
                                                    'list.$.inventory.glove':
                                                        user.inventory.glove -
                                                        1,
                                                },
                                            }
                                        )
                            ...
                            if (user.inventory.glove) {
                                const countOwner =
                                    getRandomInt(0, 2) * owner.inventory.glove;
                                const countUser =
                                    getRandomInt(0, 2) * user.inventory.glove;
                ...
                if (payload.action === 'takeRoulette') {
                    try {
                        ...
                        if (profiles.length === 1)
                            return ctx.reply(
                                '☢ Игра доступна только для бесед!'
                            );
                        ...
                        if (!existPlayer) {
                            ...
                            ctx.reply(
                                `🔫 ${user.first_name}, ты уже взял револьвер!`
                            );
                        }
                    } catch (err) {
                        if (err.response.error_code === 917) {
                            return ctx.reply(
                                '☢ Для игры, боту требуется админка!'
                            );
                        }
                        ctx.reply(
                            '☢ Блин блинский, сбой какой-то [takeRoulette]'
                        );
                    }
                }
                        ...
                        try {
                            ...
                            if (currentPlayer.bullet !== currentBullet) {
                                ...
                                if (notShotPlayers.length === 0) {
                                    ...
                                    currentRoom.roulette.players.forEach(
                                        async (player) => {
                                            await room.updateOne(
                                                {
                                                    room: conversationID,
                                                    'roulette.players.user':
                                                        player.user,
                                                },
                                                {
                                                    $set: {
                                ...
                                if (currentRoom.roulette.players.length === 1) {
                                    const winner =
                                        currentRoom.roulette.players[0];
                                    await room.updateOne(
                                        { room: conversationID },
                                        {
                                            $set: {
                            ...
                            console.error(err);
                            ctx.reply(
                                '☢ Блин блинский, сбой какой-то [rouletteShoot]'
                            );
                        }
                    });
                }
                if (payload.action === 'rouletteTop') {
                    ...
                    const formatedTopList = topList.map((player, idx) => {
                        return `${idx + 1}. ${player.first_name} ${
                            player.last_name
                        } - ${player.score}\n`;
                    });
                    ctx.reply(
                ...
                if (payload.action === 'takeCards') {
                    try {
                        ...
                        let cardOne =
                            arCards21[getRandomInt(0, arCards21.length)];
                        let cardTwo =
                            arCards21[getRandomInt(0, arCards21.length)];
                ...
                if (payload.action === 'takeCard') {
                    try {
                        ...
                        const arDelPlayer = arPlayers.filter(
                            (el) => el.user !== userID
                        );
                        const arDelRoom = rooms.filter(
                            (el) => el.room !== conversationID
                        ...
                        console.error(err);
                        ctx.reply(
                            '&#9762; Блин блинский, сбой какой-то [takeCard]'
                        );
                    }
                }
                if (payload.action === 'giveTop') {
                    ...
                    const arTopPlayerList = arTopPlayers.map((el, idx) => {
                        return `${idx + 1}. ${el.firstName} ${el.lastName} - ${
                            el.score
                        }\n`;
                    });
                    return ctx.reply(
                ...
                if (payload.action === 'showCards') {
                    try {
                        ...
                        console.error(err);
                        ctx.reply(
                            '&#9762; Блин блинский, сбой какой-то [showCards]'
                        );
                    }
                }
